  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-24">
      ...
      <h2 className="text-2xl font-black tracking-tight text-stone-900 dark:text-stone-100">
        Scan. Reveal. <span className="text-red-600 dark:text-red-500">Switch.</span>
      </h2>
          ...
          {scanState === "idle" && (
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="text-xs font-semibold text-stone-500 hover:text-red-600 dark:text-stone-400"
              >
            ...
            <div className="mt-4 rounded-xl border border-stone-200 bg-white p-4 dark:border-stone-800 dark:bg-stone-900">
              ...
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  className="search-input flex-1 rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm font-mono dark:border-stone-700 dark:bg-stone-800 dark:text-stone-100"
                />
                <button
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className="rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs font-medium text-stone-600 transition-all hover:border-red-300 hover:text-red-600 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-400 dark:hover:border-red-700"
                >
                  {label}
                </button>
              ))}
      ...
      {/* ═══ NOT FOUND ═══ */}
      {scanState === "not-found" && (
        <div className="mt-6 rounded-xl border border-stone-200 bg-white p-6 text-center dark:border-stone-800 dark:bg-stone-900">
...
function ScanResult({ brand, onReset }: { brand: Brand; onReset: () => void }) {
  ...
  return (
    <div className="mt-6 space-y-4">
      {/* The reveal */}
      <div className="reveal-up rounded-xl border-2 border-red-200 bg-red-50 p-5 dark:border-red-900 dark:bg-red-950/30">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="mt-0.5 text-xl font-black text-stone-900 dark:text-stone-100">
              {brand.name}
            </h3>
            {parent && (
              <Link
                href={`/corporation/${parent.slug}`}
                className="mt-0.5 inline-block text-sm font-bold text-red-600 hover:underline dark:text-red-400"
              >
      ...
      {/* Who profits */}
      {parent && (
        <div className="reveal-up reveal-delay-1 rounded-xl border border-stone-200 bg-white p-4 dark:border-stone-800 dark:bg-stone-900">
          <p className="text-xs font-bold uppercase tracking-wider text-stone-400 dark:text-stone-500">
          ...
          <div className="mt-3 space-y-2">
            {parent.owners
              ...
              .map((owner, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg bg-stone-50 px-3 py-2 dark:bg-stone-800">
                  <div>
          ...
          <Link
            href={`/corporation/${parent.slug}`}
            className="mt-3 block text-center text-xs font-bold text-red-600 hover:underline dark:text-red-400"
          >
      ...
      {parent && parent.issues.length > 0 && (
        <div className="reveal-up reveal-delay-2 rounded-xl border border-stone-200 bg-white p-4 dark:border-stone-800 dark:bg-stone-900">
          <p className="text-xs font-bold uppercase tracking-wider text-stone-400 dark:text-stone-500">
            Known issues — {parent.name}
          </p>
          <div className="mt-3 space-y-2">
            {parent.issues.slice(0, 3).map((issue, i) => (
              <div key={i} className="rounded-lg bg-stone-50 px-3 py-2 dark:bg-stone-800">
      ...
      {alternatives.length > 0 && (
        <div className="reveal-up reveal-delay-3">
          <p className="mb-2 text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-500">
            Make the switch
          </p>
      ...
      <button
        onClick={onReset}
        className="w-full rounded-lg border-2 border-stone-300 py-3 text-sm font-bold text-stone-700 transition-all hover:border-red-500 hover:text-red-600 active:scale-[0.97] dark:border-stone-700 dark:text-stone-300"
      >
